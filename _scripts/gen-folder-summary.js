...
dirs.forEach((dir) => {
  ...
  if (dirStat.isDirectory()) {
    fs.readdirSync(path.join(cwd, dir)).forEach((n) => {
...
`
    ...
    if (hasFormerFile) {
      const oldContentLines = fs
        .readFileSync(outputFileName)
        .toString()
        .split('\n')
      for (
        let lineNumber = 0;
        lineNumber < oldContentLines.length;
        lineNumber++
      ) {
        const lineContent = oldContentLines[lineNumber]
        if (lineContent.indexOf(GEN_END_MARK) > -1) {
          content =
            generatedContent + oldContentLines.slice(lineNumber + 1).join('\n')
          break
        }
      }
    }
